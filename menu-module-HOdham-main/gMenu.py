#returns the value in CAPITALS. Driver called for lowercase.  (-1)  Otherwise nicely done.
#you can keep it this way for your future projects.

#this does all validation
def getUserInput(choices, menuItems):
    userChoice = input(f"Enter your choice : ").upper()

    while userChoice not in choices and (len(userChoice) == 0 or userChoice[0].upper() not in choices):
    ...
    if len(userChoice) > 0 and userChoice[0].upper() in choices:
        ...
        return cap(menuItems[choices.find(userChoice)])
